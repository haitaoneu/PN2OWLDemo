...
<xsl:template name="net">
	...
	<xsl:for-each select="place">
		<place provider="petrinet.provider.place">
			...
			<graphics>
				<position>
					<xsl:attribute name="x"><xsl:value-of select="/pnml/net/place/graphic/position/@x"/></xsl:attribute>
					<xsl:attribute name="y"><xsl:value-of select="/pnml/net/place/graphic/position/@y"/></xsl:attribute>
				</position>
			</graphics>
			<initialMarking>
				<token id="viptool.petrinet.datastructure.Token@1000" provider="petrinet.provider.token">
					<value><xsl:value-of select="initialMarking"/></value>
				</token>
			</initialMarking>
		</place>
	</xsl:for-each>
	<xsl:for-each select="transition">
		<transition provider="petrinet.provider.transition">
			...
			<graphics>
				<position>
					<xsl:attribute name="x"><xsl:value-of select="/pnml/net/transition/graphic/position/@x"/></xsl:attribute>
					<xsl:attribute name="y"><xsl:value-of select="/pnml/net/transition/graphic/position/@x"/></xsl:attribute>
				</position>
			</graphics>
		</transition>
	</xsl:for-each>
